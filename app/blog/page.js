...
// Utility function to generate a random date within the past 90 days
const generateRandomPastDate = () => {
  const now = new Date();
  const pastDate = new Date(now);
  const randomDaysAgo = Math.floor(Math.random() * 90); // Random number of days within 3 months
  pastDate.setDate(now.getDate() - randomDaysAgo);
  return pastDate.toISOString(); // You can format it later
};
